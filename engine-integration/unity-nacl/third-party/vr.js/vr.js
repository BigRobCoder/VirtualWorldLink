 ...
 * predicted - if True get predicted orientation
 */
vr.DataSource.prototype.poll = function(state, predicted) {
};
 ...
 * @override
 */
vr.PluginDataSource.prototype.poll = function(state, predicted) {
  if (!this.native_) {
    return;
  }
  ...
  var pollData = this.native_.poll(predicted);
 ...
 * @override
 */
vr.DriverDataSource.prototype.poll = function(state, predicted) {
  var present = this.driver_.isPresent();
  state.hmd.present = present;
  if (present) {
    // predicted not yet supported
 ...
 */
vr.Runtime.prototype.poll = function(state, predicted) {
  // Reset.
  state.sixense.present = false;
  state.hmd.present = false;

  // Poll data.
  this.dataSource_.poll(state, predicted);
 ...
 * };
 */
vr.pollState = function(state, predicted) {
  if (!predicted) {
    predicted = false;
  }
  return vr.runtime_.poll(state, predicted);
};


// Set HMD prediction time
vr.DataSource.prototype.setHmdPredictionTime = function(dt) {
};
vr.PluginDataSource.prototype.setHmdPredictionTime = function(dt) {
  this.execCommand_(3, dt);
};
vr.setHmdPredictionTime = function(dt) {
  vr.runtime_.dataSource_.setHmdPredictionTime(dt);
};

// Set HMD acceleration gain
vr.DataSource.prototype.setHmdAccelGain = function(ag) {
};
vr.PluginDataSource.prototype.setHmdAccelGain = function(ag) {
  this.execCommand_(4, ag);
};
vr.setHmdAccelGain = function(ag) {
  vr.runtime_.dataSource_.setHmdAccelGain(ag);
}
...
vr.StereoRenderer.prototype.render = function(vrstate, callback, opt_scope,
  ...
  for (var n = 0; n < eyes.length; n++) {
    ...
    // Setup stereo maps
    if (stereoMaps) {
      for (var m = 0; m < stereoMaps.length; m++) {
        var map = stereoMaps[m];
        map.material.map = map.eyeTextures[n];
      }
    }
